...
def validate_ip(ip_string: str, allow_private: bool = True) -> bool:
    ...
    try:
        # Try to parse as IP address - this will raise exception for invalid IPs
        ...
        logger.debug(f"Invalid IP address format: {ip_string}")
        return False
    except ValueError:
        # Invalid IP address value
        logger.debug(f"Invalid IP address value: {ip_string}")
        return False
    except TypeError:
        # Wrong type passed
        logger.debug(f"Invalid IP address type: {type(ip_string)}")
        return False
    except Exception as e:
        # Any other unexpected error
        logger.debug(f"Unexpected error validating IP '{ip_string}': {e}")
        return False
...
def validate_file_path(
    file_path: Union[str, Path], must_exist: bool = False, must_be_file: bool = True
) -> bool:
    """
    Validate file path

...
def test_real_scan():
    ...
    try:
        ...
        if not executor.check_tool_exists("nmap"):
            ...
            log_info("To enable real scans, install nmap: sudo apt install nmap")
            return True

        log_info("Testing real scan on localhost...")

        # Use CLI to run a real scan with short timeout and limited ports
        result = subprocess.run(
            [
                sys.executable,
                "main.py",
        ...
        if result.returncode == 0:
            ...
            if REPORT_DIR.exists():
                ...
                if report_files:
                    ...
                    log_warning("! No report file created (might be normal)")
            else:
                log_warning("! Report directory doesn't exist")
        else:
            log_warning(f"! Real scan returned non-zero exit code: {result.returncode}")
            if result.stderr:
                log_info(f"STDERR: {result.stderr[:200]}...")
            # Don't fail the test if scan has issues, just warn
            log_info("This might be normal if localhost has no open ports")

        log_success("Real scan test completed (with or without findings)")
        return True

    except subprocess.TimeoutExpired:
        log_warning("! Real scan test timed out (this is expected behavior)")
        return True
    except Exception as e:
        log_error(f"Real scan test failed: {e}")
        log_info("This might indicate an issue with the CLI interface")
        return False
...
def main():
    """Main test function"""
    # Setup logger
    logger = LoggerSetup.setup_logger(name="project_test", level="INFO", use_rich=True)
